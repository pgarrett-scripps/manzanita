...
impl IntervalTree {
    ...
    pub fn difference(&self, other: &PyAny) -> PyResult<IntervalTree> {
        ...
        for interval in self.items() {
            if !to_subtract.iter().any(|other_iv| intervals_equal(&interval, other_iv)) {
                result.add(interval);
            }
        }
    ...
    pub fn intersection(&self, other: &PyAny) -> PyResult<IntervalTree> {
        ...
        for interval in self.items() {
            if other_intervals.iter().any(|other_iv| intervals_equal(&interval, other_iv)) {
                result.add(interval);
            }
        }
    ...
    pub fn split_overlaps(&mut self, datafunc: Option<PyObject>) -> PyResult<()> {
        ...
        for boundary in boundaries {
            ...
            let containing = self.at(point);
            if containing.iter().any(|iv| iv.begin < point && point < iv.end) {
    ...
    pub fn merge_overlaps(&mut self, datafunc: Option<PyObject>, strict: Option<bool>) -> PyResult<()> {
        ...
        intervals.sort_by(|a, b| a.begin.partial_cmp(&b.begin).unwrap_or(std::cmp::Ordering::Equal));
    ...
    pub fn merge_equals(&mut self, datafunc: Option<PyObject>) -> PyResult<()> {
        ...
        let mut groups: std::collections::HashMap<(OrderedFloat<f64>, OrderedFloat<f64>), Vec<Interval>> = std::collections::HashMap::new();
        
        for interval in intervals {
            let key = (OrderedFloat(interval.begin), OrderedFloat(interval.end));
            groups.entry(key).or_insert_with(Vec::new).push(interval);
        }
    ...
    pub fn merge_neighbors(&mut self, datafunc: Option<PyObject>, max_dist: Option<f64>, merge_overlaps: Option<bool>) -> PyResult<()> {
...
fn merge_interval_group(intervals: &[Interval], datafunc: Option<&PyObject>) -> PyResult<Interval> {
    ...
    let merged_data = if let Some(func) = datafunc {
        Python::with_gil(|py| {
            ...
            let py_intervals: Vec<PyObject> = intervals.iter()
                .map(|iv| Py::new(py, iv.clone()).unwrap().to_object(py))
                .collect();
            let interval_list = pyo3::types::PyList::new(py, &py_intervals);
            func.call1(py, (interval_list,)).unwrap_or_else(|_| intervals[0].data.clone())
        })
    } else {
